...
export const createSlides = async () => {
  ...
  slides.videos.forEach((img, index) => {
    ...
    const SlideItemImgEl = img.thumbnail
      ? isImg
...
const mountIFrameTo = (parent, vidObj) => {
  ...
  ModalSlideItemVidEl.style.aspectRatio =
    vidObj.aspect_ratio.split(':')[0] +
    ' / ' +
    vidObj.aspect_ratio.split(':')[1];
  ...
  ModelSlideItemVidLike.src = likedList.includes(vidObj.uuid)
    ? '/assets/images/heart-filled.svg'
    : '/assets/images/heart-outlined.svg';
  ...
  ModelSlideItemVidLike.addEventListener('click', (e) =>
    handleVideoLike(e, vidObj, ModelSlideItemVidLike)
  );
...
const toggleMute = (ModalSlideItemVidEl) => {
  ...
  const muteBtn =
    ModalSlideItemVidEl.parentElement.querySelector('#muteButton');
...
export const playPauseToggle = (ModalSlideItemVidEl, pause = false) => {
...
const handleVideoLike = (e, vidObj, ModelSlideItemVidLike) => {
  ...
  // If liked, they are removed from the liked array in the localstorage
    ...
    localStorage.setItem(
      'likedVideos',
      JSON.stringify([...likedList.filter((uuid) => uuid !== vidObj.uuid)])
    );
